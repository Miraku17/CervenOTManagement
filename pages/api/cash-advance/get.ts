...
async function handler(req: AuthenticatedRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (isManagingDirector || isOperationsManager || isAccounting) {
      // These positions see everything
      positionsToExclude = [];
    } else if (isHR) {
      // HR cannot see Managing Director and Operations Manager requests
    ...
    if (confidentialUserIds.length > 0) {
      ...
      console.log('Cash Advance Get - Applied filter to exclude confidential requests:', positionsToExclude);
    }
