...
export default function ImportStoreInventoryLogsModal({ isOpen, onClose }: ImportLogsModalProps) {
  ...
  const fetchImportLogs = async () => {
    setLoading(true);
    try {
      const response = await fetch('/api/inventory/import-logs');
      ...
      <div className="bg-slate-900 rounded-xl shadow-2xl w-full max-w-6xl max-h-[90vh] flex flex-col border border-slate-700">
        ...
        <div className="flex items-center justify-between p-4 sm:p-6 border-b border-slate-700">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl sm:text-2xl font-bold text-white">Store Inventory Import History</h2>
          </div>
          <button
            onClick={onClose}
