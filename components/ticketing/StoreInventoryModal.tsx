...
const StoreInventoryModal: React.FC<StoreInventoryModalProps> = ({ isOpen, onClose, onSuccess, editItem }) => {
  ...
  // Search available assets as user types (debounced)
  useEffect(() => {
    if (!isOpen || entryMode !== 'select') return;

    const searchAssets = async () => {
      setIsLoadingAssets(true);
      try {
        const query = assetSearchTerm.trim();
        const response = await fetch(`/api/assets/search-available?q=${encodeURIComponent(query)}&limit=20`);
  ...
  }, [assetSearchTerm, isOpen, entryMode]);
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (assetSearchRef.current && !assetSearchRef.current.contains(target)) {
  ...
  const handleAssetSelect = (asset: any) => {
    ...
    setAssetSearchTerm(`${asset.serial_number} - ${asset.categories?.name} ${asset.brands?.name} ${asset.models?.name}`);

    // Auto-populate fields from selected asset
    setCategory(asset.categories?.name || '');
    setSelectedCategoryId(asset.categories?.id || null);
    setBrand(asset.brands?.name || '');
    setSelectedBrandId(asset.brands?.id || null);
    setModel(asset.models?.name || '');
    setSelectedModelId(asset.models?.id || null);
    setSerialNumber(asset.serial_number || '');
    setUnderWarranty(asset.under_warranty || false);
    setWarrantyDate(asset.warranty_date || '');
      ...
      <div className="bg-slate-900 rounded-lg shadow-xl w-full max-w-4xl border border-slate-700 max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="border-t border-slate-700 pt-4 mt-4">
            ...
            {!isEditMode && (
              <div className="mb-4">
                <label className="block text-sm font-medium text-slate-300 mb-2">Entry Mode</label>
                <div className="flex gap-2">
                  ...
                  >
                    Manual Entry (Create New)
                  </button>
                </div>
              </div>
            )}

            {/* Asset Search (Select Mode) */}
            {!isEditMode && entryMode === 'select' && (
              <div ref={assetSearchRef} className="relative mb-4">
                <label htmlFor="assetSearch" className="block text-sm font-medium text-slate-300 mb-1">
                  Search Available Assets <span className="text-red-500">*</span>
                </label>
                <div className="relative">
                  <input
                    type="text"
                    ...
                    placeholder="Search by serial number, category, brand, or model..."
                    required={entryMode === 'select'}
                  />
                  <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 w-5 h-5 pointer-events-none" />
                </div>
                  ...
                  <div className="absolute z-50 w-full mt-1 bg-slate-800 border border-slate-700 rounded-md shadow-xl max-h-60 overflow-y-auto">
                    {isLoadingAssets ? (
                      ...
                      availableAssets.map((asset) => (
                        ...
                        >
                          <div>
                            <div className="text-sm font-medium text-white">{asset.serial_number}</div>
                            <div className="text-xs text-slate-400 mt-0.5">
                              {asset.categories?.name} - {asset.brands?.name} {asset.models?.name}
                            </div>
                            <div className="flex items-center gap-2 mt-1">
                              <span className="inline-flex px-2 py-0.5 rounded text-xs bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
                                Available
                              </span>
                              {asset.under_warranty && (
                                <span className="text-xs text-blue-400">Under Warranty</span>
                              )}
                            </div>
                          </div>
                        </button>
                      ...
                      <div className="px-4 py-3 text-sm text-slate-500 text-center">
                        {assetSearchTerm ? `No available assets found matching "${assetSearchTerm}"` : 'Start typing to search...'}
                      </div>
                    )}
                  </div>
                )}
            ...
            {entryMode === 'select' && selectedAsset && (
              <div className="mb-4 p-4 bg-slate-800 border border-blue-500/20 rounded-md">
                <h4 className="text-sm font-semibold text-white mb-2">Selected Asset</h4>
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div>
                    <span className="text-slate-400">Serial Number:</span>
                    <span className="text-white ml-2 font-medium">{selectedAsset.serial_number}</span>
                  </div>
                  <div>
                    <span className="text-slate-400">Category:</span>
                    <span className="text-white ml-2">{selectedAsset.categories?.name}</span>
                  </div>
                  <div>
                    <span className="text-slate-400">Brand:</span>
                    <span className="text-white ml-2">{selectedAsset.brands?.name}</span>
                  </div>
                  <div>
                    <span className="text-slate-400">Model:</span>
                    <span className="text-white ml-2">{selectedAsset.models?.name}</span>
                  </div>
                  {selectedAsset.under_warranty && (
                    <div className="col-span-2">
                      <span className="text-slate-400">Warranty Until:</span>
                      <span className="text-blue-400 ml-2">{selectedAsset.warranty_date ? new Date(selectedAsset.warranty_date).toLocaleDateString() : 'N/A'}</span>
                    </div>
                  )}
                </div>
              </div>
            ...
            {/* Device, Brand, Model Row - Only show in manual mode or edit mode */}
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div className="relative">
                  <input
                    ...
                    className={`w-full border rounded-md py-2 px-3 pr-20 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                      entryMode === 'select' && selectedAsset
                    ...
                    required
                    readOnly={entryMode === 'select' && !!selectedAsset}
                  />
                  {selectedCategoryId && (
                    <button
                      type="button"
            ...
            {/* Serial Number Row - Only show in manual mode or edit mode */}
            {(entryMode === 'manual' || isEditMode) && (
              <div className="mt-4">
              ...
              <input
                ...
                className={`w-full border rounded-md py-2 px-3 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                  entryMode === 'select' && selectedAsset
                    ? 'bg-slate-900 border-slate-700 text-slate-400 cursor-not-allowed'
                    : 'bg-slate-800 border-slate-700 text-white'
                }`}
